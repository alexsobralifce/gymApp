...
- **RF04 - Gestão de Fichas de Treino & Construtor Mobile-First**:
  - **Professor/Academia**: Criar, editar, excluir e enviar treinos para alunos. Clonagem individual ou em lote para múltiplos alunos. Marcar fichas como templates reutilizáveis. Suporte a métodos avançados (`TRADICIONAL`, `BI_SET`, `TRI_SET`, `DROP_SET`, `REST_PAUSE`, `PIRAMIDE`, `CIRCUITO`), tempo de descanso customizado por exercício e observações técnicas.
  - **Aluno**: Criar treinos próprios em autogestão, editar ou excluir treinos salvos (mesmo recebidos de professores, sem apagar histórico de execução). Seleção de métodos de treino e descanso.
  - **Experiência Mobile-First com Drawer & Didática PT-BR**: Construtor fluido com Biblioteca de Exercícios em Bottom-Sheet Drawer (`ExerciseLibraryDrawer`), busca instantânea com `useDeferredValue`, botões touch com área mínima de 44px, feedback visual de exercícios já adicionados, reordenação simplificada e inputs numéricos otimizados (`type="tel" inputMode="numeric"`). Modal de prévia didática (`ExercisePreviewModal`) com GIF animado, nome 100% em português, músculos primários/secundários trabalhados, passo a passo numerado de execução (`passos_pt`), explicação conceitual (`descricao_pt`) e dicas de segurança/performance (`dica`). Badges visuais temáticos de métodos (`WorkoutMethodBadge`) e conectores de blocos conjugados.
...
- **RF06 - Execução e Monitoramento de Treino**: Cronômetro com tempo decorrido, timer flutuante de descanso entre séries com presets e progresso, campos de carga/repetição por série com auto-preenchimento da última carga e botões de incremento (±2,5kg), coach marks direcionais na primeira execução (`gymapp_first_workout_done`), modal de confirmação ao sair com cancelamento e reset do cronômetro (`POST /treinos/:id/cancelar`), conclusão em lote por exercício ("✓ Concluir Exercício"), modal de avaliação pós-treino completo com nota em estrelas (1 a 5 ⭐), campo de feedback textual para o professor, percepção de esforço (Fácil/Moderado/Intenso/Muito Intenso), resumo metabólico (calorias e FC). Notificação push dual automática ao Professor quando o aluno avalia a sessão. Vibração + notificação local ao fechar/minimizar o app com treino em andamento (`EM_EXECUCAO`), com debounce de 8s (não vibra em bloqueios breves de tela) — hook `useIncompleteWorkoutReminder`.
