...
export default function PostCard({ post, onCurtir, onDescurtir, onComentar }: PostCardProps) {
  ...
  const [curtidasCount, setCurtidasCount] = useState(post.curtidasCount)
  ...
  async function handleComentar() {
    ...
    try {
      ...
      setComentarios((prev) => [
        { id: String(Date.now()), postId: post.id, alunoId: '', autorNome: 'Voce', texto: novoTexto.trim(), criadoEm: new Date().toISOString() },
        ...prev,
      ])
      setNovoTexto('')
    } catch { /* ok */ }
    ...
    <div className="rounded-2xl bg-surface-card border border-surface-input p-4 animate-fade-in">
      ...
      <div className="flex items-center gap-3 mb-3">
        ...
        <div className="min-w-0 flex-1">
          <p className="text-sm font-semibold text-text">{post.autorNome}</p>
          <p className="text-xs text-text-muted">{tempoRelativo(post.criadoEm)}</p>
        </div>
      ...
      {/* Grupo muscular */}
      {post.grupoMuscularResumo && (
        <p className="text-xs text-text-muted mb-2">
          Grupos: {post.grupoMuscularResumo}
        </p>
      )}
      ...
      <div className="flex items-center gap-4 pt-2 border-t border-surface-input">
        ...
        >
          ...
          {post.comentariosCount > 0 && <span>{post.comentariosCount}</span>}
        </button>
      </div>
            ...
            comentarios.map((c) => (
              <div key={c.id} className="flex gap-2">
                <div className="flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-surface-input text-[10px] font-bold text-text-muted">
                  {c.autorNome.slice(0, 2).toUpperCase()}
                </div>
                <div className="min-w-0">
                  <p className="text-xs font-semibold text-text">{c.autorNome}</p>
