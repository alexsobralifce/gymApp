...
import { NotFoundError, ConflictError, LimiteProfessoresExcedidoError, ForbiddenError, } from '../../../domain/errors/AppError.js';
// ─── UC-05: Cadastrar academia ───────────────────────────────────────────────
export async function cadastrarAcademia(usuarioId, input) {
    const academiaExistente = await prisma.academia.findUnique({ where: { usuario_id: usuarioId } });
    if (academiaExistente)
        throw new ConflictError('Este usuário já possui uma academia cadastrada');
...
export async function aprovacaoAcademia(academiaId, acao, motivo) {
    const academia = await prisma.academia.findUnique({ where: { id: academiaId } });
    if (!academia)
        throw new NotFoundError('Academia');
...
export async function definirLimiteProfessores(academiaId, limite) {
    const academia = await prisma.academia.findUnique({ where: { id: academiaId } });
    if (!academia)
        throw new NotFoundError('Academia');
    return prisma.academia.update({
...
export async function autorizarProfessorPrimeiraEtapa(academiaId, professorId) {
    ...
    if (!academia)
        throw new NotFoundError('Academia');
    if (academia.status !== AcademiaStatus.ATIVO)
        throw new ForbiddenError('Academia inativa');
...
export async function aprovacaoVinculoProfessor(vinculoId, acao) {
    const vinculo = await prisma.professorAcademia.findUnique({ where: { id: vinculoId } });
    if (!vinculo)
        throw new NotFoundError('Vínculo professor-academia');
...
export async function removerProfessor(academiaId, professorId) {
...
export async function dashboardAlunosAcademia(academiaId) {
...
export async function alterarStatusAcademia(academiaId, status) {
    const academia = await prisma.academia.findUnique({ where: { id: academiaId } });
    if (!academia)
        throw new NotFoundError('Academia');
    return prisma.academia.update({
...
//# sourceMappingURL=AcademiaService.js.map