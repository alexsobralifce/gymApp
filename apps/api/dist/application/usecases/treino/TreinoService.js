...
export async function criarTreino(professorId, input) {
...
export async function criarTreinoAutogestao(alunoId, input) {
    const aluno = await prisma.aluno.findUnique({ where: { id: alunoId } });
    if (!aluno)
        throw new NotFoundError('Aluno');
    if (aluno.professor_id !== null)
        throw new TenantAccessError();
...
export async function enviarTreinoParaAceite(treinoId, professorId) {
...
export async function responderTreino(treinoId, alunoId, acao) {
    const treino = await prisma.treino.findUnique({ where: { id: treinoId } });
    if (!treino)
        throw new NotFoundError('Treino');
    if (treino.aluno_id !== alunoId)
        throw new TenantAccessError();
...
export async function iniciarTreino(treinoId, alunoId) {
    const treino = await prisma.treino.findUnique({ where: { id: treinoId } });
    if (!treino)
        throw new NotFoundError('Treino');
    if (treino.aluno_id !== alunoId)
...
export async function registrarExecucao(treinoId, alunoId, input) {
    const treino = await prisma.treino.findUnique({ where: { id: treinoId } });
    if (!treino)
        throw new NotFoundError('Treino');
    if (treino.aluno_id !== alunoId)
...
export async function finalizarTreino(treinoId, alunoId) {
    const treino = await prisma.treino.findUnique({ where: { id: treinoId } });
    if (!treino)
        throw new NotFoundError('Treino');
    if (treino.aluno_id !== alunoId)
...
export async function dashboardProfessor(professorId) {
...
//# sourceMappingURL=TreinoService.js.map