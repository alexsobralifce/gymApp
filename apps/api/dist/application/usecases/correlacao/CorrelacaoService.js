...
function getSemanaIso(data) {
    ...
    const semana = Math.ceil(((quinta.getTime() - new Date(ano, 0, 1).getTime()) / 86400000 + new Date(ano, 0, 1).getDay() - 1) / 7);
    return `${ano}-W${String(semana).padStart(2, '0')}`;
}
async function agregarVolumeSemanal(alunoId) {
...
// ─── Cálculo principal ────────────────────────────────────────────────────────
export async function calcularEAtualizar(alunoId) {
    ...
    const deltasPeso = [];
    const deltasBf = [];
    const deltasMassaMagra = [];
    const volumesPeriodo = [];
    const pontos = [];
...
function interpretarPearson(r) {
    ...
    if (abs >= 0.7)
        return r > 0 ? 'Correlação positiva forte' : 'Correlação negativa forte';
    if (abs >= 0.5)
        return r > 0 ? 'Correlação positiva moderada' : 'Correlação negativa moderada';
    if (abs >= 0.3)
        return r > 0 ? 'Correlação positiva fraca' : 'Correlação negativa fraca';
    return 'Correlação desprezível';
}
export async function obterCorrelacoes(alunoId) {
...
//# sourceMappingURL=CorrelacaoService.js.map